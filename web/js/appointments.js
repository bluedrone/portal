...
function getUpcomingAppointments() {
  ...
  $.post("app/getUpcomingAppointments", {data:jsonData}, function(data) {
    ...
    upcomingAppointments = parsedData.appointments;
...
function getPastAppointments() {
  ...
  $.post("app/getPastAppointments", {data:jsonData}, function(data) {
    var parsedData = $.parseJSON(data);
    pastAppointments = parsedData.appointments;
    RenderUtil.render('simple_data_table', 
...
function apptRequest() {
  ...
  if($("#appt-request-clinician").val().length < 1) { 
    util_showError('#appt-request-clinician-validation');
    isValid = false;
  }
  if($("#appt-request-would-see").val().length < 1) { 
    util_showError('#appt-request-would-see-validation');
    isValid = false;
  }
  if($("#appt-request-visit-reason").val().length < 1) { 
    util_showError('#appt-request-visit-reason-validation');
    isValid = false;
  }
  ...
  var jsonData = JSON.stringify({ 
    ...
    wouldSee: $('#appt-request-would-see').val(), 
